...
## 5.4 젠킨스로 CI/CD 구현하기
젠킨스 홈 화면에서 `새로운 Item`을 클릭해 새로운 아이템을 만든다.
그런데 아이템이란 무엇일까?
젠킨스에서 아이템이란 새롭게 정의할 작업을 의미한다. 젠킨스가 CI/CD 도구임을 익히 들어서 알고 있지만,
사실 CI/CD를 하려면 각각의 작업은 모두 정의가 필요하다. 만약 작업을 코드로 정의한 경우라고 해도 작업 순서 정도는 알려줘야 한다.
모든 작업의 정의와 순서를 모아 둔 전체 작업을 프로젝트라고 하는데, 이 프로젝트를 생성하는 방식은 FreeStyle, Pipeline 등이 있다.
이렇게 프로젝트를 정의하고 생성하는 것을 아이템이라고 하며, 프로젝트 외에 실제로 작업에 도움읻되는 내용들을 정의하는 것도 아이템을 생성한다고 할 수있다.
 
1. FreeStyle project: 스타일의 자유도가 높은 방식으로, 브라우저에 사용자가 직접 설정값과 수행할 동작을 입력할 수 있다.
화면에 보이는 항목을 입력하면서 구성할 수 있어서 젠킨스와 관련된 경험이 부족한 사용자도 구성하기 쉽다는 장점이 있다.
하지만 복잡한 작업을 구성하기 어렵고 FreeStyle로 생성한 아이템은 입력한 항목의 명세서를 별도로 저장하는 과정이 없으므로 작성한 내용을 공유하기 어렵다.

2. Pipeline: 젠킨스에서 지원하는 고유의 Pipeline 문법으로 코드를 작성해 작업을 정의하는 프로젝트이다. FreeStyle과 비교해 사용법을 사전에 숙지해야 한다는 점 때문에 비교적 진입장벽이 있다.
그렇지만 변수의 정의, 반복문, 조건문 등의 프로그래밍 기법을 사용할 수 있어 좀 더 복잡한 방식의 작업을 정의하는 것이 가능하다.
또한 작성한 코드를 통해 새로운 프로젝트를 바로 생성할 수 있고 코드를 일부만 수정해 재사용하기도 수월하다.
깃허브와 같은 코드 저장소에 애플리케이션 코드를 올릴 때 Pipeline 코드로 작성한 파일을 함께 올려 두면 애플리케이션 코드와 배포 방법을 함께 관리할 수 있기 때문에 관리 편의성도 높다.

3. Multi-configuration project: 하나의 소스 코드를 여러 조건의 서버에 배포하는 프로젝트이다.

4. Folder: 젠킨스의 작업이 늘어나면 늘어날수록 단순하게 관리하기 어려운데 이런 경우 관련있는 작업들을 분류해둘 필요가 있다. 이럴 경우 분류 가능한 디렉터리를 생성하는 것이 Folder이다.

5. Multibranch Pipeline: Multibranch Pipeline을 사용하면 하나의 소스 코드 저장소 내에 존재하는 각 브랜치에서 젠킨스 파이프라인 코드가 작성된 파일을 불러와 한 번에 여러 브랜치에 대해 품질 검증, 테스트, 빌드 등의 작업을 할 수 있도록 해준다.

### 5.4.1 FreeStyle로 간단히 echo-ip 배포하기
CD를 위해서 kubectl create와 expose를 FreeStyle 프로젝트에서 배포로 사용하도록 정의하겠다.
echo-ip를 간단하게 살펴보자.

1. Dockerfile: echo-ip 도커 이미지를 빌드하는데 사용하는 파일이다. 도커 이미지를 만들기 위한 기초 이미지는 nginx:stable을 사용하고 인증과 관련된 파일과 설정 파일을 복사한 후 실행하도록 구성돼 있다.
2. Jenkinsfile: pipeline 실습을 위해 작성된 파일이다.
3. README.md: 깃허브의 초기 화면에서 안내 메세지를 표시하기 위해 사용하는 파일이다.
4. cert.crt: echo-ip의 Nginx에서 Https 접속 시 사용하는 인증서 파일이다.
5. nginx.conf: echo-ip의 응답을 설정하기 위한 Nginx 설정 파일이다. 이 설정 파일은 접속자의 IP와 응답할 수 있는 간단한 설정을 포함하고 있다.

젠킨스 FreeStyle로 CI/CD를 구성하는 순서를 요약해보겠다.
1. 깃허브에서 echo-ip를 빌드할 정보가 담긴 파일들을 내려 받는다.
2. 받은 파일들을 이용해서 컨테이너 이미지를 빌드한다.
3. 빌드한 이미지를 레지스트리(192.168.1.10:8443)에 저장한다.
4. 레지스트리에 저장한 이미지를 쿠버네티스 클러스터에 디플로이먼트로 생성하고 로드밸런서 서비스로 노출한다.

순서대로 FreeStyle 프로젝트를 진행해보자.
1. 이름을 dpy-fs-dir-prod로 지정하고 FreeStyle project를 선택한후 ok를 누른다.
2. General 탭에서 Restrict where this project can be run 체크를 해제 한다.
이 설절은 젠킨스의 에이전트가 특정한 레이블을 가지고 있을 때 해당 레이블을 가진 에이전트에서만 실행될 수 있또록 제한을 가하는 옵션인데, 현재는 불필요하다.
3. 소스 코드 관리 탭에서 젠킨스의 외부에 있는 소스 코드 저장소를 젠킨스 CI로 사용하도록 지정할 수 있다.
설정을 위해 Git을 클릭해 저장소 외에 항목들을 입력할 수 있는 메뉴로 변경된 것을 확인한다.
그리고 Repository URL에 `https://github.com/iac-source/echo-ip` 를 입력하고 Branch Specifier에는 현재 입력한 깃허브 저장소에 존재하는 주요 브랜치 변경 내용에 대해서만 CI를 진행하도록 `*/main`으로 변경한다.
4. Build 단계를 추가한다. 실제로 젠킨스 작업을 수행할 방법을 선택하는 단계이다. Add build step을 클릭해 Execute Shell을 선택한다.
이 항목에 입력한 셸 명령어로 빌드 작업이 수행된다.
5. 젠킨스에서 빌드에 사용할 명령어를 확인하고 입력한다. 명령어는 도커 이미지 빌드, 도커 이미지 푸시, 디플로이먼트 생성, 로드밸런서를 통한 노출의 4단계로 이루어져있다.
실제로 젠킨스가 작업을 수행할 때는 위에 입력한 셸 명령어들이 수행된다. 이 스크립트는 사전에 구성돼 있으므로 cat 명령어로 스크립트를 출력한 후 복사한 다음 Execute Shell 메뉴를 누른 후에 나온 칸에 붙여넣는다.
그리고 저장 버튼을 눌러 설정한 프로젝트 내용을 저장한다.
6. 저장 버튼을 누르면 그림과 같이 프로젝트 화면으로 돌아간다. Build Now를 눌러 저장한 프로젝트를 실행한다.
7. CI/CD 작업을 수행하면 Build History에 작업이 추가 된다.
작업이 정상적으로 종료됐다면 파란색 표시를 볼 수 있다. 만약 작업이 실패했다면 빨간색 표시가 나타난다.
8. 성공적으로 CI/CD 작업이 수행돼 파란색 원이 생성됐는지 확인한다. 작업 내용을 자세히 확인하고 싶다면 파란색 원 뒤 #1 누르고 빌드 관련 작업 내용을 살펴보는 메뉴로 이동한다.
빨간색 원이라면 CI/CD 작업 중 문제가 발생한 것으로 이와 동일한 과정을 거친 뒤에 빌드 작업과 관련되 내용을 확인하고 문제 되는 부분을 수정해야 한다.
9. 여기서 빌드에 관련된 자세한 내용을 살펴보려면 Console Output 메뉴를 클릭해야 한다.
콘솔 출력을 확인하면 작업의 진척 상황이나 작업의 실패 원인을 파악할 수 있다.
10. 쿠버네티스 클러스터에 디플로이먼트와 로드밸런서가 정상적으로 배포됐는지 확인한다.
```shell
kubectl get deployments
```
```shell
NAME         READY   UP-TO-DATE   AVAILABLE   AGE
fs-echo-ip   1/1     1            1           4m16s
jenkins      1/1     1            1           21h
```
...
```shell
NAME             TYPE           CLUSTER-IP      EXTERNAL-IP    PORT(S)          AGE
fs-echo-ip-svc   LoadBalancer   10.97.49.118    192.168.1.12   8080:30564/TCP   4m50s
jenkins          LoadBalancer   10.97.25.67     192.168.1.11   80:31575/TCP     21h
jenkins-agent    ClusterIP      10.106.147.36   <none>         50000/TCP        21h
kubernetes       ClusterIP      10.96.0.1       <none>         443/TCP          22h
```
디플로이먼트 fs-echo-ip와 로드밸런서 fs-echo-ip-svc가 정상적으로 배포된 것을 확인할 수 있다.

11. 좀 더 확실하게 호스트 노트북에서 웹 브라우저를 열어 192.168.1.12:8080을 입력하고 배포된 파드의 정보가 화면에 출력되는지 확인한다.
배포가 정상적으로 이루어졌다면 이 부분에 문제가 없으므로 echo-ip 애플리케이션 배포에 대해서는 따로 웹을 통해 확인하지 않겠다.

12. 다음 실습을 위해 젠킨스로 배포한 로드밸런서 서비스와 디플로이먼트를 삭제한다.
```shell
kubectl delete service fs-echo-ip-svc
kubectl delete deployment fs-echo-ip
```
13. 젠킨스 홈 화면에서 dpy-fs-dir-prod 프로젝트도 삭제한다.

### 5.4.2 Pipeline 프로젝트로 손쉽게 echo-ip 배포하기
젠킨스의 Pipeline은 연속적인 작업을 코드 또는 파일로 정의해주는 젠킨스 기능이다.
Pipeline은 고유의 문법으로 작성된 코드 또는 이러한 내용을 담고 있는 파일로 이루어져 있다.
파이프라인 문법을 통해 젠킨스는 코드로 작성한 내용이 실제로 동작하는 작업이 되는 코드로서의 파이프라인을 구현할 수 있다.

CI/CD는 빌드-테스트-패키징-배포 드으이 여러 단계로 나누어진 작업들이 효과적으로 이루어져야 한다.
FreeStyle의 경우 화면에서 메뉴를 눌러서 정의하는 방식이기 때문에 여러 사람들에게 전달하려면 사용법을 책자로 만들고 교육해야 한다.
또한 일부 내용이 변경되면 다시 책자를 만들거나 변경된 내용을 교육해야 한다.
그래서 젠킨스에서는 Pipeline을 통해 CI/CD 내용을 코드 또는 파일로 정의해 단순히 해당 코드 또는 파일을 가져다 쓰면 모든 것이 쉽게 되도록 지원한다.

젠킨스 Pipeline은 크게 2가지의 문법으로 코드를 작성할 수 있다. 첫 번째는 스크립트 문법이고, 두 번째는 선언적인 문법이다.
각 문법은 일반적인 경우에는 큰 차이점이 없으나, 쿠버네티스상에서 젠킨스 에이전트를 설정할 때 스크립트 문법을 사용하면 익숙하지 않은 젠킨스의 고유 문법으로 작성해야 한다.
하지만 선언적 문법을 사용하면 우리가 이미 익숙한 야믈을 그대로 사용할 수 있으므로 쿠버네티스상의 젠킨스 에이전트 설정에는 선언적 문법을 사용하는 것을 권장한다.

실습을 진행하기 전에 Pipeline으로 어떻게 CI/CD를 구현하는지 간략히 정리해보자.
1. 깃허브와 같은 소스 코드 저장소에서 빌드할 소스 코드와 젠킨스 내부의 작업을 선언적인 문법으로 정의해 둔 Jenkinsfile을 내려받는다.
2. 내려받은 Jenkinsfile을 해석해서 작성자의 의도에 맞는 작업을 자동으로 수행한다. 이때 Jenkinsfile을 통해 이루어지는 것은 5.4.1에 FreeStyle로 진행한 내용과 같다.
이를 간편하게 Jenkinsfile로 정의해둔 것이다.

1. Pipeline 배포 실습을 위해 젠킨스 홈 화면으로  이동한 후 새로운 Item 버튼을 눌러 새로운 아이템을 생성하는 화면으로 넘어간다.
2. 이번에 선택할 아이템은 Pipeline이며 생성할 프로젝트의 이름은 dpy-pl-bulk-prod이다.
3. General 탭은 프로젝트의 일반적인 설정을 기록하는 곳이다. 프로젝트의 설명 및 빌드 작업의 동작 방식에 대한 여러 설정이 있지만 이와 관련된 설정을 하지 않아도 실습을 진행하는데 문제가 없으므로 기본 설정을 사용 하겠다.
4. Build Trigger 탭은 빌드를 유발하기 위한 조건을 설정하는 탭이다. 사용자의 환경에 따라 빌드에 필요한 조건이 달라질 수 있기 때문에 존재하는 설정이다. 탭에서 제공하는 옵션을 살펴보자.
   * Build after other projects are built: 다른 프로젝트를 빌드한 이후에 이 프로젝트를 빌드 한다. 특정 프로젝트를 빌드하기 위한 사전 조건을 구성해야 하는 경우 혹은 여러 개의 프로젝트를 빌드할 때 순서에 따른 의존 관계가 있는 경우에 유용하다.
   * Build periodically: 주기적으로 프로젝트 빌드를 수행한다. 일정 주기로 빌드를 수행하는 경우에 사용할 수 있다. 예를 들어 매일 최신 버전의 소프트웨어를 배포하는 방식을 야간 빌드라고 하는데 이러한 작업을 위해 1주일 주기로 작업을 설정할 수 있다. 주기를 설정할 때 크론이라는 스케줄 도구의 문법을 활용해 작성한다.
   * Poll SCM: 깃허브 등의 소스 코드 저장소에서 주기적으로 내용을 검사해 빌드한다. Poll SCM은 빌드를 수행하기 전 소스 코드 저장소의 내용에 변경이 있는지 확인한다. 이후 변경이 있을 때만 빌드를 수행한다.
   * 빌드 안함: 빌드를 사용하지 않는다. 임시로 사용하지 않을 프로젝트 등에 설정할 수 있다. 이 옵션을 설정하면 웹 화면상 '이 프로젝트는 현재 비활성 상태입니다.'라는 안내가 표시된다.
   * Quiet period: 빌드를 실행할 때 약간의 지연 시간을 주는 옵션이다. 지연 시간의 범위 이내에서 들어온 요청은 한 건으로 처리하기 때문에 불필요한 작업의 중복을 방지할 수 있다. 예를 들어 Quiet period를 5초로 설정한다면, 여러 번의 푸시가 들어오더라도 5초 내에 들어온 마지막 푸시만을 실행한다.
   단 젠킨스 UI의 Build Now는 즉시 작업을 수행하기 때문에 이 값을 생략한다. 깃허브와 같은 소스 코드 저장소와 연계해 외부의 요청으로부터 작업이 수행되는 경우에 적용할 수 있다.
   * 빌드를 원격으로 유발: 외부와 연계를 위해 젠킨스의 빌드 작업을 외부에서 URL을 호출해 시작할 때 사용한다. 이 옵션을 선택하면 실행 권한의 인증을 위한 토큰을 입력할 수 있다.
   토큰을 설정한 경우 <JENKINS_URL>/job/<작업명>/build?token=<토큰 이름>의 형식으로 URL을 호출하면 빌드 작업이 시작된다. 이 주소는 주로 깃허브의 푸시 또는 메신저의 웹훅과 같이 주소를 이용해서 빌드를 시작할 수 있는 곳에서 사용 된다.
5. Advanced Project Options 탭은 프로젝트의 고급 옵션을 설정하는 곳으로 젠킨스의 플러그인 설치에 따라 생성된다. 이번 실습에서는 별도의 고급 옵션을 설정하는 플러그인이 설치되지 않은 환경이다.
6. Pipeline 탭에서는 젠킨스의 빌드 작업 절차를 정의할 수 있다. 빌드 작업을 위한 스크립트를 직접 입력하거나 외부의 소스 코드 저장소에서 선언적인 문법으로 작성된 파일을 가지고와서 빌드 작업을 수행할 수 있따.
Definition에 Pipeline script를 선택할 경우 해당 화면에서 Freestyle과 같이 직접 입력한 내용을 사용한다. 여기서 차이점은 선언적인 문법을 사용해야 한다는 것이다.
7. 이제 외부 저장소에서 선언적인 문법으로 작성된 파일을 가지고 오기 위해서 다음과 같이 지정한다. SCM은 Git으로 설정하고 Repository URL을 `https://github.com/iac-source/echo-ip` 로 설정한다.
그리고 파일을 가지고 올 브랜치를 설정하기 위해 Branch Specifier를 `*/main`으로 설정한다. 젠킨스에서 처리할 작업 정보를 지닌 파일이 Script Path에 Jenkinsfile(기본값)로 입력돼 있는 것을 확인한다.
참고로 SCM은 소스 코드 관리를 의미하며, SCM은 깃허브를 대중적으로 가장 많이 사용한다.
8. 이제 저장을 눌러 Pipeline 프로젝트가 저장된 것을 확인한다.
9. 저장한 Pipeline 프로젝트를 빌드하기 전에 SCM을 통해 깃허브 저장소에서 가지고 오는 Jenkinsfile의 소스코드를 살펴보고, 코드를 이용해서 어떻게 Pipeline 프로젝트가 동작하는지 확인해보겠다.
   * pipeline: 선언적인 문법이 싲가하는 부분이다. 선언적인 문법으로 작성된 작업들은 pipeline {} 사이에 작업 내용을 작성해야 한다.
   * agent: 작업을 수행할 에이전트를 지정하고 필요한 설정을 한다. 
   지정된 에이전트 내부에서 젠킨스 빌드 작업이 실제로 수행되는데 다음과 같은 여러 가지 방식으로 지정할 수 있다.
     1. any: 사용가능한 에이전트를 임의로 지정
     2. label: 특정한 레이블과 일치하는 에이전트 노드를 지정
     3. docker: 에이전트 노드의 이미지를 도커로 지정
     4. kubernetes: 에이전트 노드를 쿠버네티스 파드로 지정
   * stages: stage들을 모아서 정의하고 이를 순서대로 진행하게 해준다.
   * steps: step들을 정의하는 영역이다. stage는 괄호 안에 여러 개의 step들을 정의할 수 있는데 이 step들 내부에서 실제로 동작하는 내용들이 정의 된다.
   그리고 젠킨스에서 빌드가 진행될 때 stage 별로 진행 단계를 확인할 수 있다.
   * step: stage 내부에서 실제 작업 내용을 작성하는 영역이다. stage 내부에 여러 step이 존재할 수 있다. step 영역 내부에서 script, sh, git과 같은 작업을 통해서 실제로 동작하게 된다.
...
```
* 4~8 라인: 소스 코드를 저장소인 깃허브로부터 소스 코드를 내려받은 단계이다. 이때 소스코드를 내려받기 위해 git 작업을 사용한다. git 작업에서 인자로 요구하는 git url은 깃허브 저장소의 주소이고 branch는 main 브랜치를 설정했다.
* 9~16 라인: 도커 명령을 이용해 컨테이너 이미지를 빋르하고, 빌드한 이미지를 레지스트리에 저장하는 작업을 수행하는 단계이다. 이때 sh 작업을 통해 docker 명령을 사용한다.
* 17~24 라인: kubectl 명령으로 전 단계에서 레지스트리에 저장한 이미지를 pl-bulk-prod 디플로이먼트로 배포하고, 배포한 디플로이먼트를 로드밸런서 타입으로 노출하는 작업을 수행한다. 이때 sh 작업을 통해 kubectl 명령을 사용한다.
10. 이제 다시 젠킨스 화면으로 돌아가 Build Now를 클릭해 젠킨스의 빌드 및 배포 작업을 시작하겠다.
11. 각 스테이지 별로 배포가 정상적으로 완료된 것을 확인했으며 실제로 쿠버네티스 클러스터에 디플로이먼트와 로드밸런서가 정상적으로 배포됐는지 확인한다.
```shell
kubectl get deployments
```
```shell
...
```shell
NAME               TYPE           CLUSTER-IP      EXTERNAL-IP    PORT(S)          AGE
jenkins            LoadBalancer   10.97.25.67     192.168.1.11   80:31575/TCP     23h
jenkins-agent      ClusterIP      10.106.147.36   <none>         50000/TCP        23h
kubernetes         ClusterIP      10.96.0.1       <none>         443/TCP          23h
pl-bulk-prod-svc   LoadBalancer   10.111.247.97   192.168.1.12   8080:32183/TCP   43s
```
12. 이제 다음 실습을 위해 서비스와 디플로이먼트를 삭제한다.
```shell
